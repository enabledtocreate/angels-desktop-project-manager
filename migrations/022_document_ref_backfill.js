...
async function up() {
  ...
  for (const row of rows) {
    if (String(row.doc_type || '').trim().toLowerCase() !== 'changelog') continue;
    changelogByProject.set(row.project_id, parseJson(row.editor_state, null));
  }
  ...
  for (const row of rows) {
    ...
    const normalized = normalizeDocumentEditorStateForStorage(project, row.doc_type, withBackfill);
